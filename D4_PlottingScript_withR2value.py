...
def logistic5(x, a, b, c, d, g):
    # example of 4PL fit: http://people.duke.edu/~ccc14/pcfb/analysis.html
    y = d + ((a-d) / (np.power((1 + np.power((x/c), b)), g)))
    return y
...
def fitData(x, y, blanks):
    ...
    # compute R^2 value
    ydata_fit = sigmoid(xdata, *popt)
    r2val = r2Value(ydata, ydata_fit)
    #print r2val
    plt.figure()
    plotResiduals(xdata, ydata, ydata_fit)
    ...
    plt.ylim(-0.05, 1.05)
    plt.legend(loc='best')
    plt.text(1.55, 0, r'$R^2 = $' + str(round(r2val,4)))
    plt.show() 
    return
